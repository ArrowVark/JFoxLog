...
public class FoxgloveChannel<T extends FoxgloveLoggable> {
    ...
    public FoxgloveChannel<T> logCacheDataAccessDates(boolean value) {
        logAccessDate = value;
        if (value) {
            FoxgloveDebugPanel.log("\"" + topic + "\" has been set to log cache and data access dates");
        } else {
            FoxgloveDebugPanel.log("\"" + topic + "\" has been set to not log cache and data access dates");
        }
        return this;
    }
